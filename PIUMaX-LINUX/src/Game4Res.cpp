...
CGame4Res::CGame4Res(void)
{
	for(int x = 0; x < SIZE_GA4RES; x++)
...
int CGame4Res::LoadSkin(void)
{
	int nDur[SIZE_GA4RES], nFrames[SIZE_GA4RES];
	FILE* hFile;
	TCHAR str[260];
...
void CGame4Res::Update(DWORD dwMiliseconds)
{
	for(int x = 0; x < SIZE_GA4RES; x++)
		skin[x]->Update(dwMiliseconds);
}

void CGame4Res::Delete()
{
	for(int x = 0; x < SIZE_GA4RES; x++)
		skin[x]->DeleteImage();
}
