...
bool CMovieStream2::OpenAVI(LPCTSTR szFile)
{
	...
	if(nwidth > MOVIESTREAM_MAXRWIDTH) nwidth = GetPowerOf2(MOVIESTREAM_MAXRWIDTH);
	if(nheight > MOVIESTREAM_MAXRHEIGHT) nheight = GetPowerOf2(MOVIESTREAM_MAXRHEIGHT);
...
void* WINAPI CMovieStream2::DecodeFrame(void*)
{
	...
	if(nDifFrame > 0)
	{
		...
		//if(dwTime > dwTimeAVFrame) dwTimeAVFrame = dwTime;
		//dwTime = SDL_GetTicks();
		sws_scale (img_convert_ctx, pFrame->data, pFrame->linesize,
					0, pCodecCtx->height,
					pFrameRGB->data,pFrameRGB->linesize);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, nwidth,  nheight, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		//dwTime = SDL_GetTicks()-dwTime;
		//if(dwTime > dwTimeAVScale) dwTimeAVScale = dwTime;
		av_free_packet(&packet);
	}
	bBusy = false;
	return (void*)NULL;
