...
CGame5Res::CGame5Res(void)
{
	for(int x = 0; x < SIZE_GA5RES; x++)
...
int CGame5Res::LoadSkin(void)
{
	int nDur[SIZE_GA5RES], nFrames[SIZE_GA5RES];
	FILE* hFile;
	TCHAR str[260];
...
void CGame5Res::Update(DWORD dwMiliseconds)
{
	for(int x = 0; x < SIZE_GA5RES; x++)
		skin[x]->Update(dwMiliseconds);
}

void CGame5Res::Delete()
{
	for(int x = 0; x < SIZE_GA5RES; x++)
		skin[x]->DeleteImage();
}
