...
CGame1Res::CGame1Res(void)
{
	for(int x = 0; x < SIZE_GA1RES; x++)
...
int CGame1Res::LoadSkin(void)
{
	int nDur[SIZE_GA1RES], nFrames[SIZE_GA1RES];
	FILE* hFile;
	TCHAR str[260];
...
void CGame1Res::Update(DWORD dwMiliseconds)
{
	for(int x = 0; x < SIZE_GA1RES; x++)
		skin[x]->Update(dwMiliseconds);
}

void CGame1Res::Delete()
{
	for(int x = 0; x < SIZE_GA1RES; x++)
		skin[x]->DeleteImage();
}
