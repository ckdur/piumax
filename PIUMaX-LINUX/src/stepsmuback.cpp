...
FMOD_CHANNEL     *channel_mine;
FMOD_SOUND* sound_mine;
FMOD_CHANNEL     *channel_s;
FMOD_RESULT       res_step;
FMOD_SOUND* music_s;
CMovieStream2 *backMovie;
CSKSFView* viewp1 = new CSKSFView();
CSKSFView* viewp2 = new CSKSFView();
CSKSFDoc* doc;
...
void CSKSFView::Draw()
{
	...
	// ** Timing
	if(g_bIsMusic) m_nNext = (int)(nPos)+m_nOffset;
	else m_nNext += dwTick - dwLastTick;
	...
	if(m_bLoadMachine && m_bCharacter && m_nCharacter > 0)
	{
		...
		for(int w = 0; w < (m_bIsSingle?1:2); w++)
		{
			...
			glPushMatrix();
			glLoadIdentity();
			if(m_bIsSingle && piuconfig.nModeMach && !np[int(!(m_nPlayer-1))]) glTranslated(17.51965, 0.0, 0.0);
			else
			{
	...
	int nMin[3], nMax[3];
	for(int m = 0; m < 3; m++) GetLimitsOfDrawing(dBeat, m, &nMin[m], &nMax[m]);

	m_nSizeArrows = 55.0/8.0;
	...
	if(m_bLeftAttack && m_bIsSingle)
	{
		...
		glTranslated(1.4*m_nSizeArrows+10.0, 0.0, 0.0);
	}
	...
	if(m_bIsSingle && piuconfig.nModeMach && !np[int(!(m_nPlayer-1))]) glTranslated(21.5, 0.0, 0.0);
	...
	if(!m_bFreedom)
	{
		for(int i = 0; i < nLimitX; i++)
		{
			int x = nOrder[i];
			t = 0;
			l = x<5?(x*m_nSizeArrows*0.9):(4*m_nSizeArrows*0.9+m_nSizeArrows+(x-5)*m_nSizeArrows*0.9);
			...
			glBegin(GL_QUADS);
			glColor4d(1.0, 1.0, 1.0, 1.0);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.09-OffsetInZArrow(x));
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.09-OffsetInZArrow(x));
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.09-OffsetInZArrow(x));
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.09-OffsetInZArrow(x));
			glEnd();
		}
		for(int i = 0; i < nLimitX; i++)
		{
			...
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.08-OffsetInZArrow(x));
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.08-OffsetInZArrow(x));
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.08-OffsetInZArrow(x));
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.08-OffsetInZArrow(x));
			glEnd();
		}
	}
	...
	for( int n = 0; n < 3; n++)
	{
		...
		int* nPArrowIn;	// Almacenar los indices dentro del juego
		nPArrowIn = (int*)calloc(0, sizeof(int));
		int nSizePai = 0;
		int nArrowBegin = 0;
			...
			if(bCountThis)
			{
				...
				if(dOffset < -m_dLevelMach)	// Esta flecha se ha perdido
				{
					for(int x = 0; x < nLimitX; x++)
					{
						...
						/*if(LOWORD(GetArrow(x, y, n)) >= 2 && LOWORD(GetArrow(x, y, n)) <= 4)
							SetArrowStatus(x, y, n, 1);	// Contar MISS PARA LOS FRIZZES SI SE PERDIERON*/
					}
					// Aqui cuenta un miss
					if(SomeArrow(y, n) && m_bIsArrowsMisible[n])
					{
				...
				{
					nSizePai++;
					nPArrowIn = (int*)realloc(nPArrowIn, nSizePai*sizeof(int));
					nPArrowIn[nSizePai-1] = y;
					if(dMinusOffset >= abs(dOffset) && SomeNote(y, n))
					{
						dMinusOffset = abs(dOffset);
						nPArrow = y;
						nArrowBegin = nSizePai-1;
					}
				}
				...
				for(int x = 0; x < nLimitX; x++)
				{
					...
					if(LOWORD(dwArrow) >= 2 && LOWORD(dwArrow) <= 4 && evarrows.bArrows[x] && (dUbi-dBeatnStop) < (m_dLevelMach/4)) // Frizz
					//if(LOWORD(dwArrow) >= 2 && LOWORD(dwArrow) <= 4 && (evarrows.bArrows[x] || (dPressBeat[x]-dUbi)>=0.0) && (dUbi-dBeatnStop) < (m_dLevelMach/4)) // Frizz
					//if(LOWORD(dwArrow) >= 2 && LOWORD(dwArrow) <= 4 && ((dPressBeat[x]-dUbi)>=0.0) || (evarrows.bArrows[x] && ((dUbi-dBeatnStop) < (m_dLevelMach/4)))) // Frizz
					{
						//SetArrowStatus(x, y, n, 5);	// Si.. se cogio la parte del frizz
						bPicked[x] = true;
						//bCount = true;
				...
				if(SomeArrow(y, n)) if(abs(dOffset) <= m_dLevelMach && bCount && bTake) // Esta entre el perfect y el bad
				{
					if(abs(dOffset) <= (1*m_dLevelMach/4) && abs(dOffset) >= 0)  // Perfect
					{
						for(int x = 0; x < 10; x++)
						{
							if(LOWORD(GetArrow(x, y, n)) >= 2 && LOWORD(GetArrow(x, y, n)) <= 4)
								SetArrowStatus(x, y, n, 5);	// Contar PERFECT PARA LOS FRIZZES
						}
						status.nPerfect[n]++;
					...
					for(int x = 0; x < 10; x++) // Asumir que la ultima vez que se presionó fue en el comienzo
						if(bPicked[x]) evarrows.nArrows[x] = m_nOffset;
				}
			}
		}
		int* nOrderP;
		if(nSizePai) nOrderP = new int[nSizePai];
		int nSign = 0;
		int nCarIsd = -1;
		int nOperIsd = 0;
		for(int u = 0; u < nSizePai; u++)
		{
			int nArrowCarIso = nPArrowIn[u];
			int nNum = 0;
			do
			{
				nCarIsd++;
				nSign = ((nCarIsd%2)?(-1):(1));
				nOperIsd = (nCarIsd+1)/2;
				nNum = nArrowBegin+(nSign*nOperIsd);
			}while(nNum >= nSizePai);
			nOrderP[u] = nNum;
		}
		//if(nPArrow != -1)
		for(int u = 0; u < nSizePai; u++)
		{
			...
			{*/
			int nCombo;
			double dUbi = BeatWArrow(nPArrow, n);//songc.dWWArrows1[nPArrow];//GetDocument()->BeatArrow(nPArrow);
			double dOffset = 0.0;
			bool bCount = false;
			bool bPicked[] = {false,false,false,false,false,false,false,false,false,false};
			for(int x = 0; x < nLimitX; x++)
			{
				...
				if(LOWORD(dwArrow) >= 2 && LOWORD(dwArrow) <= 4 && evarrows.bArrows[x] && (dUbi-dBeatnStop) < (m_dLevelMach/4)
				...
				if(LOWORD(dwArrow) > 4 && abs(dPressBeat[x]-dUbi)<=m_dLevelMach && GetArrowStatus(x, nPArrow, n) != 5) // Objetos
				{
					bPicked[x] = true;
					...
					case 5:	// Minas
						...
						FMOD_System_PlaySound(sys, FMOD_CHANNEL_FREE, sound_mine, 0, &channel_mine);
						break;
					case 6:	// Pociones
						status.nItems++;
						status.dLife += double(HIWORD(dwArrow))*dIncrBar;
						status.nPotions++;
						break;
					case 7:	// Corazones
						status.nItems++;
				...
				if(LOWORD(dwArrow) && off >= dOffset) dOffset = off;
			}
			if(dOffset <= m_dLevelMach && bCount == true) // Esta entre el perfect y el bad
			{
				if(dOffset <= m_dLevelMach && dOffset > (3*m_dLevelMach/4)) // Bad
				{
					for(int x = 0; x < nLimitX; x++)
					{
						if(LOWORD(GetArrow(x, nPArrow, n)) == 1)
				...
				if(dOffset <= (3*m_dLevelMach/4) && dOffset > (2*m_dLevelMach/4)) // Good
				{
					for(int x = 0; x < nLimitX; x++)
					{
						if(LOWORD(GetArrow(x, nPArrow, n)) == 1)
				...
				if(dOffset <= (2*m_dLevelMach/4) && dOffset > (1*m_dLevelMach/4)) // Great
				{
					for(int x = 0; x < nLimitX; x++)
					{
						if(LOWORD(GetArrow(x, nPArrow, n)) == 1)
				...
				if(dOffset <= (1*m_dLevelMach/4) && dOffset >= 0)  // Perfect
				{
					for(int x = 0; x < nLimitX; x++)
					{
						if(LOWORD(GetArrow(x, nPArrow, n)) == 1)
		...
		for(int i = 0; i < nLimitX; i++)
		{
			...
			{
				int nSkin = 0;
				if(GetArrowStatus(x, y, n) < 4)
				{
					DWORD dwDArrow = LOWORD(GetArrow(x, y, n));
					bool bActiveX = true;
					if(dwDArrow == 1 || dwDArrow >= 5)
					{
						...
						glBegin(GL_QUADS);
						glColor4d(1.0, 1.0, 1.0, o);
						glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.05-OffsetInZArrow(x));
						glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.05-OffsetInZArrow(x));
						glColor4d(1.0, 1.0, 1.0, ob);
						glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0501-OffsetInZArrow(x));
						glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0501-OffsetInZArrow(x));
						glEnd();
					}
					if(dwDArrow >= 2 && dwDArrow <= 4)
					{
						...
						for(j = y; j < nLim; j++)
						{
							if(LOWORD(GetArrow(x, j, n)) == 4 || GetArrowStatus(x, j+1, n)>3)
							{
								break;
							}
						}
						...
						sl = x<5?(x*m_nSizeArrows*0.9):(4*m_nSizeArrows*0.9+m_nSizeArrows+(x-5)*m_nSizeArrows*0.9);
						...
						sb = st+(m_bDrop?1:-1)*m_nSizeArrows;
						sr = sl+m_nSizeArrows;
						if((dwDArrow==2 || dwDArrow==4) && k != y)
						{
							double dAccOff = BeatArrow(y-1, n) - dBeat;
							l = sl;
							if(GetArrowStatus(x, y-1, n) >= 4 && evarrows.bArrows[x] && (BeatArrow(y, n)-dBeat) >= 0.0) t = (m_bDrop?1:-1)*m_nSizeArrows;
							else
							{
								t = (BeatArrow(y-1, n)-dBeat)*m_nDistArrows;
								ModifyPosArrows(&l, &t);
								if(y-1 == k) t=sb;
							}
							b = (BeatArrow(y, n)-dBeat)*m_nDistArrows;
							ModifyPosArrows(&r, &b);
							if(GetArrowStatus(x, y-1, n) >= 4 && evarrows.bArrows[x] && (BeatArrow(y, n)-dBeat) >= 0.0 && ((m_bDrop?1:-1)*b) < (m_nSizeArrows/2)) b = (m_bDrop?1:-1)*m_nSizeArrows/2;
							if(((m_bDrop?1:-1)*b) < ((m_bDrop?1:-1)*st+m_nSizeArrows/2))
								b = st+(m_bDrop?1:-1)*m_nSizeArrows/2;
							if(((m_bDrop?1:-1)*t) < ((m_bDrop?1:-1)*st+m_nSizeArrows/2))
							...
							glBegin(GL_QUADS);
							glColor4d(1.0, 1.0, 1.0, o);
							glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0502-OffsetInZArrow(x));
							glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0502-OffsetInZArrow(x));
							glColor4d(1.0, 1.0, 1.0, ob);
							glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0502-OffsetInZArrow(x));
							glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0502-OffsetInZArrow(x));
							glEnd();
							if(GetArrowStatus(x, y-1, n) >= 4 && evarrows.bArrows[x] && (BeatArrow(y, n)-dBeat) >= 0.0)
							{
								l = sl;
								t = (m_bDrop?1:-1)*m_nSizeArrows/2.0;
								b = t+(m_bDrop?1:-1)*m_nSizeArrows/2.0;
								r = sr;
								SetOffset(&l, &t, &r, &b);
								if(m_bTrainingMode)
								{
								...
								glBegin(GL_QUADS);
								glColor4d(1.0, 1.0, 1.0, 1.0);
								glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.03-OffsetInZArrow(x));
								glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.03-OffsetInZArrow(x));
								glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.04-OffsetInZArrow(x));
								glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.04-OffsetInZArrow(x));
								glEnd();
								t = 0;
								b = t+(m_bDrop?1:-1)*m_nSizeArrows;
								l = sl;
								...
								glBegin(GL_QUADS);
								glColor4d(1.0, 1.0, 1.0, 1.0);
								glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.02-OffsetInZArrow(x));
								glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.02-OffsetInZArrow(x));
								glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.03-OffsetInZArrow(x));
								glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.03-OffsetInZArrow(x));
								glEnd();
							}
						}
						if(j==y)
						{
							...
							if(((m_bDrop?1:-1)*t) < ((m_bDrop?1:-1)*st+m_nSizeArrows/2))
								t = st+(m_bDrop?1:-1)*m_nSizeArrows/2;
							double dAc = 1-abs(b-t)/m_nSizeArrows;
							DeriveOpacityPos(t, &o, n);
							DeriveOpacityPos(b, &ob, n);
							DeriveOpacityPos2(t, &o, y, n);
							DeriveOpacityPos2(b, &ob, y, n);
							...
							glBegin(GL_QUADS);
							glColor4d(1.0, 1.0, 1.0, o);
							glTexCoord2d(0.0, 1-dAc); glVertex3d(l, t, -50.05-OffsetInZArrow(x));
							glTexCoord2d(1.0, 1-dAc); glVertex3d(r, t, -50.05-OffsetInZArrow(x));
							glColor4d(1.0, 1.0, 1.0, ob);
							glTexCoord2d(1.0, 0.0); glVertex3d(r, b, -50.0501-OffsetInZArrow(x));
							glTexCoord2d(0.0, 0.0); glVertex3d(l, b, -50.0501-OffsetInZArrow(x));
							glEnd();
						}
						if(k==y)
						{
							...
							glBegin(GL_QUADS);
							glColor4d(1.0, 1.0, 1.0, o);
							glTexCoord2f(0.0f, 1.0f); glVertex3d(sl, (st+sb)/2, -50.0501-OffsetInZArrow(x));
							glTexCoord2f(1.0f, 1.0f); glVertex3d(sr, (st+sb)/2, -50.0501-OffsetInZArrow(x));
							glColor4d(1.0, 1.0, 1.0, ob);
							glTexCoord2f(1.0f, 0.0f); glVertex3d(sr, sb, -50.0502-OffsetInZArrow(x));
							glTexCoord2f(0.0f, 0.0f); glVertex3d(sl, sb, -50.0502-OffsetInZArrow(x));
							glEnd();
							// Flecha
							glBindTexture(GL_TEXTURE_2D, skin[nSkin]->skin[x%5]->GetCurrentTex());
							glBegin(GL_QUADS);
							glColor4d(1.0, 1.0, 1.0, o);
							glTexCoord2f(0.0f, 1.0f); glVertex3d(sl, st, -50.05-OffsetInZArrow(x));
							glTexCoord2f(1.0f, 1.0f); glVertex3d(sr, st, -50.05-OffsetInZArrow(x));
							glColor4d(1.0, 1.0, 1.0, ob);
							glTexCoord2f(1.0f, 0.0f); glVertex3d(sr, sb, -50.0501-OffsetInZArrow(x));
							glTexCoord2f(0.0f, 0.0f); glVertex3d(sl, sb, -50.0501-OffsetInZArrow(x));
							glEnd();
						}
					}
				}
				...
				if(bsc.dwBSAnswers[y*10+x])
				{
					...
					if(bsc.nDBSAnswers[y] == 1 && dwGArrow == 1) // Es un número, y debemos mostrarlo como tal
					{
						...
						for(int z = 0; z < str.GetLength(); z++)
						{
							...
							glBegin(GL_QUADS);
							glColor4d(1.0, 1.0, 1.0, o);
							glTexCoord2f(0.0f, 1.0f); glVertex3d(ll, t, -50.04-OffsetInZArrow(x));
							glTexCoord2f(1.0f, 1.0f); glVertex3d(rr, t, -50.04-OffsetInZArrow(x));
							glColor4d(1.0, 1.0, 1.0, ob);
							glTexCoord2f(1.0f, 0.0f); glVertex3d(rr, b, -50.0401-OffsetInZArrow(x));
							glTexCoord2f(0.0f, 0.0f); glVertex3d(ll, b, -50.0401-OffsetInZArrow(x));
							glEnd();
						}
					}
					else if(dwGArrow == 0)
					{
						glBindTexture(GL_TEXTURE_2D, skin[2]->skin[x]->GetCurrentTex());
						glBegin(GL_QUADS);
						glColor4d(1.0, 1.0, 1.0, o);
						glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.05-OffsetInZArrow(x));
						glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.05-OffsetInZArrow(x));
					...
					else if(dwGArrow == 18)
					{
						...
						{
							glBindTexture(GL_TEXTURE_2D, skin[0]->skin[x]->GetCurrentTex());
							glBegin(GL_QUADS);
							glColor4d(1.0, 1.0, 1.0, o);
							glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0502-OffsetInZArrow(x));
							glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0502-OffsetInZArrow(x));
							glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0503-OffsetInZArrow(x));
							glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0503-OffsetInZArrow(x));
							glEnd();
						}
						// Segunda asignación
						o = 1.0-o;
	...
	for(int i = 0; i < nLimitX; i++)
	{
		...
		if(nCaret <= 500)
		{
			...
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l-dCaret*1, t+(m_bDrop?-1:1)*dCaret, -50.031-0.001*((double)(10-i)));
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r+dCaret*1, t+(m_bDrop?-1:1)*dCaret, -50.031-0.001*((double)(10-i)));
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r+dCaret*1, b-(m_bDrop?-1:1)*dCaret, -50.031-0.001*((double)(10-i)));
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l-dCaret*1, b-(m_bDrop?-1:1)*dCaret, -50.031-0.001*((double)(10-i)));
			glEnd();
		}
		...
		if(nCaret <= 500)
		{
			...
			glBegin(GL_QUADS);
			glColor4d(1.0, 1.0, 1.0, 1.0-dCaret);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.021-0.001*((double)(10-i)));
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.021-0.001*((double)(10-i)));
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.021-0.001*((double)(10-i)));
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.021-0.001*((double)(10-i)));
			glEnd();
			glBlendFunc(GL_SRC_ALPHA, GL_ONE);
			glBindTexture(GL_TEXTURE_2D, skin[0]->skin[x%5+30]->GetCurrentTex());
			glBegin(GL_QUADS);
			glColor4d(1.0, 1.0, 1.0, 1.0-dCaret);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l-m_nSizeArrows-dCaret*m_nSizeArrows/2, t+(m_bDrop?-1:1)*(m_nSizeArrows+dCaret*m_nSizeArrows/2), -50.011-0.001*((double)(10-i)));
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r+m_nSizeArrows+dCaret*m_nSizeArrows/2, t+(m_bDrop?-1:1)*(m_nSizeArrows+dCaret*m_nSizeArrows/2), -50.011-0.001*((double)(10-i)));
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r+m_nSizeArrows+dCaret*m_nSizeArrows/2, b-(m_bDrop?-1:1)*(m_nSizeArrows+dCaret*m_nSizeArrows/2), -50.011-0.001*((double)(10-i)));
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l-m_nSizeArrows-dCaret*m_nSizeArrows/2, b-(m_bDrop?-1:1)*(m_nSizeArrows+dCaret*m_nSizeArrows/2), -50.011-0.001*((double)(10-i)));
			glEnd();
			glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		}
	}
	...
	glPopMatrix();
	glPushMatrix();
	if(m_bIsSingle && piuconfig.nModeMach && !np[int(!(m_nPlayer-1))]) glTranslated(0.5, 0.0, 0.0);
	else
	{
		if(m_bIsSingle && m_nPlayer == 1)
		{
	...
	int nShowCombo = bMissCombo?(status.nMissCombo[0]+status.nMissCombo[1]+status.nMissCombo[2]):(status.nCombo[0]+status.nCombo[1]+status.nCombo[2]);
	if(status.nTypeEva>0 && status.nTypeEva<=5)
	{
		if(nCaret >= 0 && nCaret <= 100)
		{
			...
			glBegin(GL_QUADS);
			glColor4d(1.0, 1.0, 1.0, 1.0);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50);
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50);
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50);
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50);
			glEnd();
			if(nShowCombo >= 4)
			{
				l = -7.6;
	...
	if(status.dLife > 1.0) status.dLife = 1.0;
	...
	for(int y = 0; y < bsc.nSizeBSPopUp; y++)
	{
		...
		if(nOff>=-200 && nOff<0)
		{
			...
			glBegin(GL_QUADS);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l-5*dCaret, t+5*dCaret, -50);
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r+5*dCaret, t+5*dCaret, -50);
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r+5*dCaret, b-5*dCaret, -50);
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l-5*dCaret, b-5*dCaret, -50);
			glEnd();
		}
		if(nOff>=0 && nOff<3000)
		{
			...
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50);
			glEnd();
			bsf->glDrawText(t, l, b, r, 0.0, 0.0, -50.0, 2.0, 5.0, strBSDisp[(int)min(bsc.dwBSPopUp[y], 49)], DT_VCENTER|DT_CENTER);
		}
		...
		if(nOff>=3200 && nOff<3400)
		{
			...
			glColor4d(1.0,1.0,1.0,1-dCaret);
			glBegin(GL_QUADS);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l+5-10*dCaret, t-5+10*dCaret, -50);
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r-5+10*dCaret, t-5+10*dCaret, -50);
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r-5+10*dCaret, b+5-10*dCaret, -50);
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l+5-10*dCaret, b+5-10*dCaret, -50);
			glEnd();
		}
	}
	//////////////////
	// Dibujo eva BS
	glPushMatrix();
	if(m_bIsSingle && piuconfig.nModeMach && !np[int(!(m_nPlayer-1))]) glTranslated(0.0, 0.0, 0.0);
	else
	{
...
{
	status.nScore -= fcombo(status.nMissCombo[0]/100)*((status.nMissCombo[0]-1)%100+1);
	if(status.nScore < 0) status.nScore = 0;
}

void CSKSFView::PlusCombo(void)
{
	status.nScore += fcombo(status.nCombo[0]/100)*((status.nCombo[0]-1)%100+1);
}
...
int CSKSFView::LoadTextures(void)
{
	if(!m_bTrainingMode)
	{
		if(m_bRandomSkin)
		{
			if(songc.nSizeArrows1) skin[0]->LoadSkin(rand()%17);
			if(songc.nSizeArrows2) skin[1]->LoadSkin(rand()%17);
			if(songc.nSizeArrows3) skin[2]->LoadSkin(rand()%17);
		}
		else
		{
			...
			if(songc.nSizeArrows2) skin[1]->LoadSkin(GetDocument()->song.hStep.nSkin2);
			if(songc.nSizeArrows3) skin[2]->LoadSkin(GetDocument()->song.hStep.nSkin3);
		}
	}
	else
	{
...
double CSKSFView::TimeToBeat(__int64 dwMiliseconds, int* nStop, double* dModStop, double* dActualBPM)
{
	...
	{
		int yj=0,ys=0;
		double dSBeat = dBeat+nMiliseconds*songc.dChangeBPM[x]/60000;
...
{
	(*l) -= 4*m_nSizeArrows*0.9+m_nSizeArrows;
	(*r) -= 4*m_nSizeArrows*0.9+m_nSizeArrows;
	if(m_bIsSingle)
	{
		(*l) -= (40.0-4*m_nSizeArrows*0.9+m_nSizeArrows)/4;
		(*r) -= (40.0-4*m_nSizeArrows*0.9+m_nSizeArrows)/4;
	}
	if(!m_bDrop)
	{
			...
			if(GetDocument()->SomeNote(y, n))
			{
				...
				for(int i = 0; i < (m_bIsSingle?5:10); i++)
				{
					...
					if(LOWORD(dwArrow) == 2 && LOWORD(dwAdd) != 4)
					{
						...
						int i = 0;
						int q = 0;
						for(int i = GetDocument()->song.hStep.nSizeTickcount-1; i >= 0; i--)
						{
						...
						for(int j = 1; j < nTickCount; j++)
						{
							...
							case 1:
								if((a+j) >= songc.nSizeArrows1)
								{
									...
									songc.dWArrows2=(double*)realloc(songc.dWArrows2, songc.nSizeArrows2*sizeof(double));
									songc.dWWArrows2=(double*)realloc(songc.dWWArrows2, songc.nSizeArrows2*sizeof(double));
									for(int z = 0; z < 10; z++) songc.dwArrows1[(songc.nSizeArrows2-1)*10+z] = 0;
								}
							...
							case 2:
								if((a+j) >= songc.nSizeArrows1)
								{
									...
									songc.dWArrows3=(double*)realloc(songc.dWArrows3, songc.nSizeArrows3*sizeof(double));
									songc.dWWArrows3=(double*)realloc(songc.dWWArrows3, songc.nSizeArrows3*sizeof(double));
									for(int z = 0; z < 10; z++) songc.dwArrows1[(songc.nSizeArrows3-1)*10+z] = 0;
								}
...
void CSKSFView::GQMemo1(int y, int nIndex, int nResp, int nArg1, int nArg2)
{
	...
	for(int i = 1; i < nCount; i++)
	{
		...
		bsc.nDBSAnswers[nInd*10+nFirst+nDisAr[0]] = 1;	// Mostrar numeros como tal
		for(int x = 1; x < nDis; x++)
		{
			do
			{
			...
			bsc.nDBSAnswers[nInd*10+nDisp+nFirst] = 1;	// Mostrar numeros como tal
		}
	}
}
...
void CSKSFView::GQMemo2(int y, int nIndex, int nResp, int nArg1, int nArg2)
{
	...
	for(int x = 0; x < 5; x++)
	{
		...
		bsc.dwDBSQuestionsEx[nIndex*40+nPos1[x]*2] = dwCaret[x];
	...
	for(int i = 1; i < nCount; i++)
	{
		...
		int nDisAr[] = {-1,-1,-1,-1,-1};
		nDisAr[0] = nPos1[i];
		bsc.nBSAnswers[nInd*10+nPos1[i]+nFirst] = 1;
		bsc.dwBSAnswers[nInd*10+nPos1[i]+nFirst] = MAKELONG(17, 1);
		// Appear Time (Answers)
		bsc.dwDBSQuestionsEx[nIndex*40+10+nPos1[i-1]*2] = (DWORD)BeatToTime(bsc.dWBSAnswers[nInd]);
		// Dissapear Time (Answers)
		bsc.dwDBSQuestionsEx[nIndex*40+20+nPos1[i-1]*2] = (DWORD)(BeatToTime(bsc.dWBSAnswers[nInd])+500);
...
void CSKSFView::GQAnal4(int y, int nIndex, int nResp, int nArg1, int nArg2)
{
	...
	bsc.dwDBSQuestions[nIndex*30] = MAKELONG(RespAr,4);
	// Argumentar la pregunta
...
bool CSKSFView::LoadMachine(CString m_strFile)
{
	FILE* hFile;
	TCHAR chFile[260];
	_sprintf(chFile, _T("%s"), m_strFile);
	...
	for(int y = 0; y < 2; y++)
	{
		...
		for(int x = 0; x < machine.hMachine.nSizeVtxMachine[y]; x++)
		{
			if(_fgets(ch, 500, hFile) != NULL)
			{
				int n;
				double v[3];
				if(_sscanf(ch, _T("*MESH_VERTEX %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2]) != 4) return false;
		...
		for(int x = 0; x < machine.hMachine.nSizeFaceMachine[y]; x++)
		{
			if(_fgets(ch, 500, hFile) != NULL)
			{
				...
				int f[3];
				int a[3];
				if(_sscanf(ch, _T("*MESH_FACE %d: A: %d B: %d C: %d AB: %d BC: %d CA: %d"), &n, &f[0], &f[1], &f[2], &a[0], &a[1], &a[2]) != 7) return false;
		...
		for(int x = 0; x < machine.hMachine.nSizeUVTMachine[y]; x++)
		{
			if(_fgets(ch, 500, hFile) != NULL)
			{
				int n;
				double v[3];
				if(_sscanf(ch, _T("*MESH_TVERT %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2]) != 4) return false;
		...
		for(int x = 0; x < machine.hMachine.nSizeTMachine[y]; x++)
		{
			if(_fgets(ch, 500, hFile) != NULL)
			{
				int n;
				int f[3];
				if(_sscanf(ch, _T("*MESH_TFACE %d\t%d\t%d\t%d"), &n, &f[0], &f[1], &f[2]) != 4) return false;
	...
	for(int x = 0; x < 10; x++)
	{
		_fgets(ch, 500, hFile);
		CString str;
		str.Format(_T("Characters\\Machine\\%s"), ch);
		str.Remove(_T('\n'));
		TCHAR chFile[500];
		_sprintf(chFile, 500, _T("%s"), str);
		machine.nTexTap[x] = GenTexImage(chFile);
	}
	for(int y = 0; y < 10; y++)
	{
		_fgets(ch, 500, hFile);
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*POS_TAP %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
		for(int z = 0; z < 3; z++) machine.hMachine.dPosTap[n*3+z] = v[z];
	}
	double v[3];
	_fgets(ch, 500, hFile);
	if(_sscanf(ch, _T("*POS_NOTAP_L_CZ\t%lg\t%lg\t%lg"), &v[0], &v[1], &v[2])!=3) return false;
	for(int z = 0; z < 3; z++) machine.hMachine.dPosNoTapL[z] = v[z];
	_fgets(ch, 500, hFile);
	if(_sscanf(ch, _T("*POS_NOTAP_R_CZ\t%lg\t%lg\t%lg"), &v[0], &v[1], &v[2])!=3) return false;
...
int CSKSFView::CompileTraining(void)
{
	...
	memset(trac.nArg, 0, trac.nSizeArg*10*sizeof(int));
	int nFoot = 0;
	int nPend = -1;
	for(int x = 0; x < songc.nSizeArrows1; x++)
	{
		...
		if(np > 1)
		{
			if(sp > 0)
			{
				if(sp == 1)
				{
					for(int y = 0; y < 10; y++) if(trac.nArg[x*10+y] != 0)
					{
						nFoot = trac.nArg[x*10+y]/*==1?2:1*/;
					}
				}
				else nFoot = 0;
				int f = nFoot==0?0:(nFoot==1?2:1);
				for(int y = 0; y < 10; y++) if((LOWORD(songc.dwArrows1[(x)*10+y]) == 1 || LOWORD(songc.dwArrows1[(x)*10+y]) == 3) && trac.nArg[x*10+y] == 0)
				{
					int b = x;
			...
			{
				int f = 1;
				for(int y = 0; y < 10; y++) if((LOWORD(songc.dwArrows1[(x)*10+y]) >= 1 && LOWORD(songc.dwArrows1[(x)*10+y]) <= 4) && trac.nArg[x*10+y] == 0)
				{
					int b = x;
					if(LOWORD(songc.dwArrows1[(x)*10+y]) == 3)
					{
		...
		else if((np-sp) > 0)
		{
			...
			if(nFoot == 0)
			{
				/*if(n1 <= 4 && n2 <= 4)	// Si ambas flechas pertenecen al mismo panel
				{*/
					if(n1 < n2)
						nFoot = 1;
					if(n1 > n2)
...
{
	int nPivot = -1;
	for(int y = 0; y < 10; y++) if(LOWORD(songc.dwArrows1[(x)*10+y]) == 1 || LOWORD(songc.dwArrows1[(x)*10+y]) == 3)
	{
...
{
	int nPivot = 0;
	for(int y = 0; y < 10; y++) if(LOWORD(songc.dwArrows1[(x)*10+y]) >= 1 && LOWORD(songc.dwArrows1[(x)*10+y]) <= 4)
	{
		nPivot++;
	}
	return nPivot;
...
double CSKSFView::EstimateStep(double dWWArrow)
{
	...
	dResult = double(rand())/RAND_MAX;	// [0.0, 1.0] Desfase secundario
...
bool CSKSFView::CompileCharacter(void)
{
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_DLLeg %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_DRLeg %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_ULLeg %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_URLeg %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_Hip %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_Waist %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_Chest %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_Head %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_ULArm %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_DLArm %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_URArm %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int y = 0; y < 2; y++)
	{
		...
		int n = 0;
		double v[3];
		if(_sscanf(ch, _T("*NORMAL_DRArm %d\t%lg\t%lg\t%lg"), &n, &v[0], &v[1], &v[2])!=4) return false;
	...
	for(int x = 0; x < 16; x++)
	{
		_fgets(ch, 500, hFile);
		CString str;
		str.Format(_T("Characters\\%d\\%s"), m_nCharacter, ch);
		str.Remove(_T('\n'));
		TCHAR chFile[500];
		_sprintf(chFile, 500, _T("%s"), str);
		crt.nTextures[x] = GenTexImage(chFile);
	}
...
void CSKSFView::DrawBS01(int y)
{
	...
	if((nOffIQuest - nBeat) < 800 && (nOffIQuest - nBeat) > 0)	// Intro
	{
		...
		glColor4d(1.0, 1.0, 1.0, 1.0);
		glBegin(GL_POLYGON);
		glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);
		glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);
		glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);
		glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);
		glEnd();
		glPopMatrix();
	}
				...
				&& m_nNext <= (bsc.dwDBSQuestionsEx[y*40+25+x]+1000)))
			{
				...
				for(int z = 0; z < 4; z++)
				{
					if(bsc.dwDBSQuestionsEx[y*40+4*x+z])
					{
						...
						glColor4d(1.0, 1.0, 1.0, 1.0);
						glBegin(GL_QUADS);
						glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);//x==0?-50.004:-50.005);
						glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);//x==0?-50.004:-50.005);
						glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);//x==0?-50.004:-50.005);
						glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);//x==0?-50.004:-50.005);
						glEnd();
						CString str;
						str.Format(_T("%d"), nNumber);
						...
						for(int w = 0; w < str.GetLength(); w++)
						{
							...
							glColor4d(1.0, 1.0, 1.0, 1.0);
							glBegin(GL_QUADS);
							glTexCoord2f(0.0f, 1.0f); glVertex3d(ll, t, -50.0);//x==0?-50.002:-50.003);
							glTexCoord2f(1.0f, 1.0f); glVertex3d(rr, t, -50.0);//x==0?-50.002:-50.003);
							glTexCoord2f(1.0f, 0.0f); glVertex3d(rr, b, -50.0);//x==0?-50.002:-50.003);
							glTexCoord2f(0.0f, 0.0f); glVertex3d(ll, b, -50.0);//x==0?-50.002:-50.003);
							glEnd();
						}
					}
				}
...
void CSKSFView::DrawBS03(int y)
{
	...
	if((nOffIQuest - nBeat) < 800 && (nOffIQuest - nBeat) > 0)	// Intro
	{
		...
		glColor4d(1.0, 1.0, 1.0, 1.0);
		glBegin(GL_POLYGON);
		glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);//-50.006);
		glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);//-50.006);
		glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);//-50.006);
		glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);//-50.006);
		glEnd();
		glPopMatrix();
		glPushMatrix();
		...
		glColor4d(1.0, 1.0, 1.0, 1.0);
		glBegin(GL_POLYGON);
		glTexCoord2f(0.0f, 1.0f); glVertex3d(_l, _t, -50.0);//-50.006);
		glTexCoord2f(1.0f, 1.0f); glVertex3d(_r, _t, -50.0);//-50.006);
		glTexCoord2f(1.0f, 0.0f); glVertex3d(_r, _b, -50.0);//-50.006);
		glTexCoord2f(0.0f, 0.0f); glVertex3d(_l, _b, -50.0);//-50.006);
		glEnd();
		glPopMatrix();
	}
	if((nOffIQuest - nBeat) <= 0 && (nOffEQuest - nBeat) > 0)	// Nudo
			...
			if(b <= 30.0 && b >= -20.0)	// Se encuentra en los límites
			{
				...
				if(bsc.dwDBSQuestions[y*30+x])
				{
					...
					if(dwGArrow == 1)
					{
						...
						glBegin(GL_QUADS);
						glColor4d(1.0, 1.0, 1.0, 1.0);
						glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);//-50.004);
						glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);//-50.004);
						glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);//-50.005);
						glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);//-50.005);
						glEnd();
					}
					if(dwGArrow == 1) // Es un número, y debemos mostrarlo como tal
					{
						...
						for(int z = 0; z < str.GetLength(); z++)
						{
							...
							glBegin(GL_QUADS);
							glColor4d(1.0, 1.0, 1.0, 1.0);
							glTexCoord2f(0.0f, 1.0f); glVertex3d(ll, t, -50.0);//-50.003);
							glTexCoord2f(1.0f, 1.0f); glVertex3d(rr, t, -50.0);//-50.003);
							glTexCoord2f(1.0f, 0.0f); glVertex3d(rr, b, -50.0);//-50.004);
							glTexCoord2f(0.0f, 0.0f); glVertex3d(ll, b, -50.0);//-50.004);
							glEnd();
						}
					}
					else
					{
						...
						glBegin(GL_QUADS);
						glColor4d(1.0, 1.0, 1.0, 1.0);
						glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);//-50.003);
						glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);//-50.003);
						glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);//-50.004);
						glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);//-50.004);
						glEnd();
					}
				}
			}
...
void CSKSFView::DrawBS05(int y)
{
	...
	if((nOffIQuest - nBeat) <= 0 && (nOffAQuest - nBeat) > 0)	// Nudo
	{
		...
		if((nOffIQuest - nBeat) >= -500)
		{
			...
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);//-50.003);
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);//-50.003);
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);//-50.003);
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);//-50.003);
			glEnd();
			glPopMatrix();
		}
	}
...
void CSKSFView::DrawBS07(int y)
{
		...
		for(int x = 0; x < nSize; x++)
		{
			if(m_nNext >= bsc.dwDBSQuestionsEx[y*40+x])
			{
				...
				if(dwGArrow == 1)
				{
					...
					glBegin(GL_QUADS);
					glColor4d(1.0, 1.0, 1.0, 1.0);
					glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);//-50.005);
					glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);//-50.005);
					glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);//-50.005);
					glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);//-50.005);
					glEnd();
				}
				if(dwGArrow == 1) // Es un número, y debemos mostrarlo como tal
				{
					...
					for(int z = 0; z < str.GetLength(); z++)
					{
						...
						glBegin(GL_QUADS);
						glColor4d(1.0, 1.0, 1.0, 1.0);
						glTexCoord2f(0.0f, 1.0f); glVertex3d(ll, t, -50.0);//-50.004);
						glTexCoord2f(1.0f, 1.0f); glVertex3d(rr, t, -50.0);//-50.004);
						glTexCoord2f(1.0f, 0.0f); glVertex3d(rr, b, -50.0);//-50.004);
						glTexCoord2f(0.0f, 0.0f); glVertex3d(ll, b, -50.0);//-50.004);
						glEnd();
					}
	...
	if((nOffAQuest - nBeat) <= 0 && (nOffEQuest - nBeat) > 0)	// Nudo
	{
		...
		for(int x = 0; x < nSize; x++)
		{
			...
			if(nSubject == 3 && nType == 4)
			{
				...
				for(int z = 0; z < str.GetLength(); z++)
				{
					...
					glTexCoord2f(0.0f, 1.0f); glVertex3d(ll, t, -50.0);//-50.003);
					glTexCoord2f(1.0f, 1.0f); glVertex3d(rr, t, -50.0);//-50.003);
					glTexCoord2f(1.0f, 0.0f); glVertex3d(rr, b, -50.0);//-50.003);
					glTexCoord2f(0.0f, 0.0f); glVertex3d(ll, b, -50.0);//-50.003);
					glEnd();
				}
			}
		}
		if((nOffAQuest - nBeat) >= -500 && nSubject == 3 && nType == 4)
		{
			...
			glBindTexture(GL_TEXTURE_2D, bs->skin[15]->texture[0]);
			glBegin(GL_POLYGON);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(l, t, -50.0);//-50.002);
			glTexCoord2f(1.0f, 1.0f); glVertex3d(r, t, -50.0);//-50.002);
			glTexCoord2f(1.0f, 0.0f); glVertex3d(r, b, -50.0);//-50.002);
			glTexCoord2f(0.0f, 0.0f); glVertex3d(l, b, -50.0);//-50.002);
			glEnd();
			glPopMatrix();
		}
	}
...
void CSKSFView::ModifyPosArrows2(double* l, double* t, int x, int y, int nGArrow)
{
	...
	for(int r = 0; r < effect.nSizeZigZagChanges[n]; r++)
	{
		...
		if(bTake)
		{
			...
			int nFarrot = min(8, LOWORD(effect.dwZigZagChanges[n][r]));
			int nx = x%5;
			double dt = (*t);
			dt = (m_bDrop?1:-1)*(dt);
			double dMult;
...
: b(false)
{
	...
	song.nWChangeBPM=(int*)calloc(song.hStep.nSizeChangeBPM,sizeof(int));
	song.dChangeBPM=(double*)calloc(song.hStep.nSizeChangeBPM,sizeof(double));
	song.nWBeatSplit=(int*)calloc(song.hStep.nSizeBeatSplit,sizeof(int));
	song.nBeatSplit=(int*)calloc(song.hStep.nSizeBeatSplit,sizeof(int));
	song.nIChangeVel=(int*)calloc(song.hStep.nSizeChangeVel,sizeof(int));
	song.nEChangeVel=(int*)calloc(song.hStep.nSizeChangeVel,sizeof(int));
	song.dChanges=(double*)calloc(song.hStep.nSizeChangeVel,sizeof(double));
	song.nIJumps=(int*)calloc(song.hStep.nSizeJumps,sizeof(int));
	song.nEJumps=(int*)calloc(song.hStep.nSizeJumps,sizeof(int));
	song.nWTickCount=(int*)calloc(song.hStep.nSizeTickcount,sizeof(int));
	song.nTickCount=(int*)calloc(song.hStep.nSizeTickcount,sizeof(int));
	song.nWStops=(int*)calloc(song.hStep.nSizeStops,sizeof(int));
...
int CSKSFDoc::LoadFile(LPCTSTR strFile)
{
	if(b)
	{
		...
		song.nWChangeBPM=(int*)realloc(song.nWChangeBPM, song.hStep.nSizeChangeBPM*sizeof(int));
		song.dChangeBPM=(double*)realloc(song.dChangeBPM, song.hStep.nSizeChangeBPM*sizeof(double));
		song.nWBeatSplit=(int*)realloc(song.nWBeatSplit, song.hStep.nSizeBeatSplit*sizeof(int));
		song.nBeatSplit=(int*)realloc(song.nBeatSplit, song.hStep.nSizeBeatSplit*sizeof(int));
		song.nIChangeVel=(int*)realloc(song.nIChangeVel, song.hStep.nSizeChangeVel*sizeof(int));
		song.nEChangeVel=(int*)realloc(song.nEChangeVel, song.hStep.nSizeChangeVel*sizeof(int));
		song.dChanges=(double*)realloc(song.dChanges, song.hStep.nSizeChangeVel*sizeof(double));
		song.nIJumps=(int*)realloc(song.nIJumps, song.hStep.nSizeJumps*sizeof(int));
		song.nEJumps=(int*)realloc(song.nEJumps, song.hStep.nSizeJumps*sizeof(int));
		song.nWTickCount=(int*)realloc(song.nWTickCount, song.hStep.nSizeTickcount*sizeof(int));
		song.nTickCount=(int*)realloc(song.nTickCount, song.hStep.nSizeTickcount*sizeof(int));
		song.nWStops=(int*)realloc(song.nWStops, song.hStep.nSizeStops*sizeof(int));
		song.dStops=(double*)realloc(song.dStops, song.hStep.nSizeStops*sizeof(double));
	}
	b = true;
	FILE* hFile;
	hFile = _fopen(strFile, _T("r+b")/*b*/);
	if(hFile == NULL)
		return 0;
	...
	fread(song.nWChangeBPM,	song.hStep.nSizeChangeBPM*sizeof(int), 1, hFile);
	fread(song.dChangeBPM,	song.hStep.nSizeChangeBPM*sizeof(double), 1, hFile);
	fread(song.nWBeatSplit,	song.hStep.nSizeBeatSplit*sizeof(int), 1, hFile);
	fread(song.nBeatSplit,	song.hStep.nSizeBeatSplit*sizeof(int), 1, hFile);
	fread(song.nIChangeVel,	song.hStep.nSizeChangeVel*sizeof(int), 1, hFile);
	fread(song.nEChangeVel,	song.hStep.nSizeChangeVel*sizeof(int), 1, hFile);
	fread(song.dChanges,	song.hStep.nSizeChangeVel*sizeof(double), 1, hFile);
	fread(song.nIJumps,		song.hStep.nSizeJumps*sizeof(int), 1, hFile);
	fread(song.nEJumps,		song.hStep.nSizeJumps*sizeof(int), 1, hFile);
	fread(song.nWTickCount,	song.hStep.nSizeTickcount*sizeof(int), 1, hFile);
	fread(song.nTickCount,	song.hStep.nSizeTickcount*sizeof(int), 1, hFile);
	fread(song.nWStops,		song.hStep.nSizeStops*sizeof(int), 1, hFile);
...
int LoadSong(ARGSTEP arg)
{
	...
	if(!arg.strSplash.IsEmpty() && !arg.bAutoPlay)
	{
		...
		if(uSplash != m_uNullTex)
		{
			...
			glBegin(GL_QUADS);
			glColor4d(1.0, 1.0, 1.0, 1.0);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(-40.0, 30.0, -50.0);
			glTexCoord2f(1.0f, 1.0f); glVertex3d(40.0, 30.0, -50.0);
			glTexCoord2f(1.0f, 0.0f); glVertex3d(40.0, -30.0, -50.0);
			glTexCoord2f(0.0f, 0.0f); glVertex3d(-40.0, -30.0, -50.0);
			glEnd();
			SDL_GL_SwapBuffers();
...
#ifdef UNICODE
#ifdef _WIN32
		UnicodeToAnsi(arg.strMusic, (char*)&ch);
#else
...
int LoadSongCompile(ARGSTEP arg, int nPlayer)
{
	...
	if(nPlayer == 1)
	{
		...
		if(!viewp1->StepCompile()) return 0;
		g_bPlayer[0] = true;
	}
	if(nPlayer == 2)
	{
		...
		if(!viewp2->StepCompile()) return 0;
		g_bPlayer[1] = true;
	}
	doc->CloseFile();
	delete doc;
...
{
	if(!viewp1->m_bAutoPlay && g_bPlayer[0]) viewp1->Event_View(nEvent, nKey);
	if(!viewp2->m_bAutoPlay && g_bPlayer[1]) viewp2->Event_View(nEvent, nKey);
}
...
int Draw_All()
{
	...
	{
		m_bBusy = true;
		FMOD_Channel_GetPosition(channel_s, &nPos, FMOD_TIMEUNIT_MS);
		FMOD_BOOL nIsPlaying = 0;
		FMOD_Channel_IsPlaying(channel_s, &nIsPlaying);
		if(!nIsPlaying)
		{
			m_bBusy = false;
			return 0;
		...
		if(backMovie->bLoad)
		{
			...
			glGenTextures(1, &texBGA);
			glBindTexture(GL_TEXTURE_2D, texBGA);
			glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
			glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, backMovie->nwidth, backMovie->nheight, 0, GL_RGB, GL_UNSIGNED_BYTE, backMovie->data);
			glBegin(GL_QUADS);
			glColor4d(1.0, 1.0, 1.0, 1.0);
			glTexCoord2f(0.0f, 1.0f); glVertex3d(-80.0, -60.0, -100.0);
			glTexCoord2f(1.0f, 1.0f); glVertex3d(80.0, -60.0, -100.0);
			glTexCoord2f(1.0f, 0.0f); glVertex3d(80.0, 60.0, -100.0);
			glTexCoord2f(0.0f, 0.0f); glVertex3d(-80.0, 60.0, -100.0);
			glEnd();
			glDeleteTextures(1, &texBGA);
		}
		/////////
...
{
	g_bDraw = true;
	FMOD_System_PlaySound(sys, FMOD_CHANNEL_FREE, music_s, 0, &channel_s);
	FMOD_Channel_SetPosition(channel_s, 0, FMOD_TIMEUNIT_MS);
	dwTick = SDL_GetTicks();
	dwLastTick = SDL_GetTicks();
}
