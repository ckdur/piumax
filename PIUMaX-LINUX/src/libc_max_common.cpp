...
int Utf16ToUtf32(const char16_t* pszA, wchar_t* pszW, size_t nCount)
{
	...
	cCharacters = (size_t)std::char_traits<char16_t>::length(pszA);
    cCharacters *= 2;
...
int Utf32ToUtf16(const wchar_t* pszA, char16_t *pszW, size_t nCount)
{
    ...
    cCharacters = (size_t)std::char_traits<wchar_t>::length(pszA);
    cCharacters *= 4;
...
void GetMaxCurrentDirectory(TCHAR* ch)
{
	...
	_getcwd(strDirect, sizeof(TCHAR)*MAX_PATH);
...
void GetMaxProcessDirectory(TCHAR* ch)
{
	...
	_sprintf(ch, _T("%ls%ls"), strDrive, strDir);
#endif // WIN32
