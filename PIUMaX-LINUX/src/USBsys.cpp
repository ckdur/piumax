...
#define PROC_USB_DEV "/sys/bus/usb/drivers/usb-storage"
#define SYS_SCSI_DEV "/sys/bus/usb/drivers/usb-storage/%s/host*"
#define SYS_SCSI_DEV2 "%s/target*"
#define SYS_SCSI_DEV3 "%s/%d*"
#define SYS_SCSI_TGT "%s/block/*"
...
void LinuxUSBmount(void)
{
  ...
  while((dt = readdir(dp)) != NULL) {
    ...
    if(dt->d_type != DT_DIR && dt->d_type != DT_LNK) continue;
    if(strcmp(dt->d_name, ".") == 0) continue;
    if(strcmp(dt->d_name, "..") == 0) continue;
    if(strcmp(dt->d_name, "module") == 0) continue;
    ...
    if(hosts.gl_pathc > 0) {
      ...
      for(p = hosts.gl_pathv, n = hosts.gl_pathc; n; p++, n--) {
        ...
        char* cap = strstr(*p, "/host");
        int host = 0;
        if(sscanf(cap, "/host%d", &host) != 1) continue;

        char pat2[PATH_MAX + NAME_MAX];
        snprintf(pat2, PATH_MAX + NAME_MAX, SYS_SCSI_DEV2, *p);

        glob_t hosts2;
        glob(pat2, 0, 0, &hosts2);
        if(hosts2.gl_pathc > 0) {
            char **s;
            int t;
            for(s = hosts2.gl_pathv, t = hosts2.gl_pathc; t; s++, t--) {
                char pat3[PATH_MAX + NAME_MAX];
                snprintf(pat3, PATH_MAX + NAME_MAX, SYS_SCSI_DEV3, *s, host);

                glob_t hosts3;
                glob(pat3, 0, 0, &hosts3);
                if(hosts3.gl_pathc > 0) {
                    char **q;
                    int r;
                    for(q = hosts3.gl_pathv, r = hosts3.gl_pathc; r; q++, r--) {

                        char tgtbuf[PATH_MAX + NAME_MAX];
                        snprintf(tgtbuf, PATH_MAX + NAME_MAX, SYS_SCSI_TGT, *q);
                          ...
                          for(ptr = devs.gl_pathv, c = devs.gl_pathc; c; ptr++, c--) {
                            ...
                            if(parts.gl_pathc > 0) {
                                ...
                                for(ptra = parts.gl_pathv, k = parts.gl_pathc; k; ptra++, k--) {
                                   ...
                                   int nRet;
                                   nRet = mount(tag, to_mnt, "vfat", MS_MGC_VAL | MS_NOSUID | MS_SYNCHRONOUS, "");
                                   if(nRet < 0)
                                   {
                                        printf("Cannot mount for /dev/%s. %s\n",
                                        basename(*ptra), strerror(errno));
                                   }
                                   else{
...
void LinuxUSBumount(void)
{
    ...
    for(int k = 0; k < nUsb; k++)
    {
        ...
        if(umount(chUsbAnsi))
        {
            printf("umount error: cannot %s, ", chUsbAnsi, strerror(errno));
        }
    }
    for(int k = 0; k < 2; k++)
    {
        sprintf(chUsbAnsi, "/mnt/%d", k);
        umount(chUsbAnsi);
    }
    nUsb = 0;
}
...
#define TIME_WAITTEXTNAME_USB 1500
	...
	TCHAR chn[12];
    Utf16ToUtf32(plItem[nItemShow]->chName, chn, 12);
    str = chn;
	...
	for(int x = 0; x < nLimit; x++)
	{
		...
		if(hFileAcc == NULL) // No existen los perfiles
		{
			...
			if(hFileAcc != NULL)	// Error al crear perfiles
			{
				...
				_sprintf(chNamesU[0], _T("m%.3d%.2d%.2d%.2d"),
					tmToday->tm_yday,tmToday->tm_hour,tmToday->tm_min,nCountNewProfiles);
                Utf32ToUtf16(chNamesU[0], pl->chName, 12);
				nCountNewProfiles++;
				...
				_sprintf(chNamesU[1], _T("m%.3d%.2d%.2d%.2d"),
					tmToday->tm_yday,tmToday->tm_hour,tmToday->tm_min,nCountNewProfiles);
				Utf32ToUtf16(chNamesU[1], pl->chName, 12);
				nCountNewProfiles++;
				...
				_sprintf(chNamesU[2], _T("m%.3d%.2d%.2d%.2d"),
					tmToday->tm_yday,tmToday->tm_hour,tmToday->tm_min,nCountNewProfiles);
				Utf32ToUtf16(chNamesU[2], pl->chName, 12);
				nCountNewProfiles++;
		...
		else
		{
			...
			while(_fgets(ch, 260, hFileAcc) != NULL && nCaretProf < 3)
			{
				...
				if(str.Find(_T('*')) == -1)
				{
					...
					CString stra;
					stra.Format(_T("%lsMAXsave/%ls.bin"), chUsb[x], str.c_str());
					hFilePlay = _fopen(stra, _T("rb")/*b*/);
					if(hFilePlay != NULL)
					{
						fread(plItem[nSizeItem], sizeof(PLAYER), 1, hFilePlay);
						Utf16ToUtf32(plItem[nSizeItem]->chName, (wchar_t*)chItem[nSizeItem], 12);
						//_strcpy(chItem[nSizeItem], plItem[nSizeItem]->chName);
						_strcpy(chSaveItem[nSizeItem], stra.c_str());
						fclose(hFilePlay);
						LPCTSTR chIso = str;
			...
			if(nChoosesUsb[x] < CHOOSES_USB)
			{
				...
				for(int i = 0; i < nSizeItem; i++)
				{
					if(nTrans1 >= (TIME_START_ENUM_USB+TIME_TRANSI_ENUM_USB*i) && nTrans1 < (TIME_START_ENUM_USB+TIME_TRANSI_ENUM_USB*(i+1)))
					{
						...
						CString str;
						TCHAR chn[12];
                        Utf16ToUtf32(plItem[i]->chName, chn, 12);
						if(i != 0) str = chn;
						else str = _T("[Machine]");
