...
int CSoundChannel::GetWaveData(float* wavearray, int numvalues, int channeloffset)
{
#ifdef PIUMAX_FMOD
	FMOD_RESULT result = FMOD_Channel_GetWaveData(m_channel, wavearray, numvalues, channeloffset);
	if (!ERRCHECK(result)) return 0;
#endif // PIUMAX_FMOD
	return 1;
}