...
int OpenReadImagePNG(LPCTSTR chName, unsigned char** data, int* nWidth, int* nHeight)
{
    ...
    // Quick fix for Linux (or png > 1.2)
    //#ifndef _WIN32
...
void GetDirStr(CString strFile, CString* strODir)
{
	...
	CString strFil;
	CString strExt;
	splitpath(strFile, *strODir, strFil, strExt);

	//(*strODir) = strFile.Left(max(strFile.ReverseFind('/')+1, strFile.ReverseFind('\\')+1));
}
...
CString Commify( int iNum )
{
	...
	{
		char cDigit = sNum.GetAt(sNum.length()-1-i);
		if( i!=0 && i%3 == 0 )
			sReturn = ',' + sReturn;
...
{
	while( s.size() && (s.GetAt(s.size()-1) == '\r' || s.GetAt(s.size()-1) == '\n') )
		s.erase(s.size()-1);
}
